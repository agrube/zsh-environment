...
function setjava() {
  ...
  then
    ...
    export ANT_OPTS="-Xmx1024102410241024102410241024102410241024m -DJAVA_HOME=$(cygpath -m ${JAVA_HOME}) -DASSAULT_HOME=$(cygpath -m ${ASSAULT_HOME}) -Djava.endorsed.dirs=$(cygpath -m ${ASSAULT_HOME})/lib/xalan-2.7 -DLicense.dir=$(cygpath -m ${LICENSE_DIR})"
...
funciton setant() {
  export ANT_OPTS="-Xmx1024m -DJAVA_HOME=${JAVA_HOME} -DASSAULT_HOME=${ASSAULT_HOME} -Djava.endorsed.dirs=${ASSAULT_HOME}/lib/xalan-2.7 -DLicense.dir=${LICENSE_DIR}"
  export ANT_ARGS="-s build.xml"
}
